...
public class RootLayoutController implements Initializable {
	...
	public void BuildMenus()
	{
		...
		// 3 or 4 lines of code. 56.00/57:00

		Menu betRules = new Menu();
		betRules.setText("Betting Rules");
		mb.getMenus().add(1,betRules);
				
		ToggleGroup tglGrpBetRules = new ToggleGroup();
		RadioMenuItem rmi1 = new RadioMenuItem("No Limit");
		RadioMenuItem rmi2 = new RadioMenuItem("Pot Limit");
		rmi1.setToggleGroup(tglGrpBetRules);
		rmi2.setToggleGroup(tglGrpBetRules);

		rmi1.setSelected(true);
		
		betRules.getItems().add(rmi1);
		betRules.getItems().add(rmi2);
